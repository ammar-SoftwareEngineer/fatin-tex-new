  ...
  const contactItems = [
    {
      icon: <HiOutlinePhone />,
      title: t("phone.title"),
      desc: mainBranch?.phone || siteData.contact.phone,
    },
    {
      icon: <HiOutlineEnvelope />,
      title: t("email.title"),
      desc: mainBranch?.email || siteData.contact.email,
    },
    {
      icon: <HiOutlineMapPin />,
      title: t("location.title"),
      desc: mainBranch?.address || siteData.contact.address,
    },
  ];

  return (
    <div className="bg-[#0d0b09] text-white overflow-hidden">
      <Breadcrumb items={[{ label: tNav("contact"), href: "/contact" }]} />
